...
for id in range(1):
    ...
    policy_kwargs = dict(activation_fn=torch.nn.ReLU, net_arch=[256, 128, 64])  # kwargs == keyword arguments
    model = TD3(MlpPolicy, env, verbose=1, policy_kwargs=policy_kwargs, action_noise=action_noise, target_policy_noise=0.1, train_freq=train_freq, learning_rate = 0.0001, learning_starts=200, tensorboard_log="./td3_tensorboard/")
    #model = TD3(MlpPolicy, env, verbose=1, policy_kwargs=policy_kwargs, train_freq=train_freq, learning_starts=200)

    time_start = time.time()
    model.learn(total_timesteps=50000,tb_log_name="first_run")  # we need to change the total steps with action numbers
