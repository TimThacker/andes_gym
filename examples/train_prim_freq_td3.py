...
for id in range(5):
    ...
    model = TD3(MlpPolicy, env, verbose=1, policy_kwargs=policy_kwargs, action_noise=action_noise, train_freq=train_freq, learning_rate = 0.0001, learning_starts=100*id, tensorboard_log="./td3_tensorboard/")
    #model = TD3(MlpPolicy, env, verbose=1, policy_kwargs=policy_kwargs, train_freq=train_freq, learning_starts=200)

    time_start = time.time()
    model.learn(total_timesteps=50000,tb_log_name="TD3_test_{}".fomrat(id))  # we need to change the total steps with action numbers
