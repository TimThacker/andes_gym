...
for id in range(1):
    ...
    action_noise = NormalActionNoise(mean=np.zeros(n_actions), sigma=0.002* np.ones(n_actions))
    ...
    model.learn(total_timesteps=300)  # we need to change the total steps with action numbers
