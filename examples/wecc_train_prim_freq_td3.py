    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_freq == 0:
          ...
          if len(x) > 0:
              ...
              if mean_reward > self.best_mean_reward and mean_reward > -358:
...
for id in range(5):
    ...
    model = TD3(MlpPolicy, env, verbose=1, policy_kwargs=policy_kwargs, action_noise=action_noise, train_freq=train_freq, batch_size=200, learning_starts=200, tensorboard_log="./td3_tensorboard_WECC2/")
    callback = SaveOnBestTrainingRewardCallback(id, check_freq=600, log_dir=log_dir)
    time_start = time.time()
    model.learn(total_timesteps=50000,tb_log_name="TD3_WECC2", callback=callback)  # we need to change the total steps with action numbers
