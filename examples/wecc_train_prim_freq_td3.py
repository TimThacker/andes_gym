...
for id in range(1):
    ...
    action_noise = NormalActionNoise(mean=np.zeros(n_actions), sigma= id * 0.0002* np.ones(n_actions))
    ...
    model.learn(total_timesteps=60000)  # we need to change the total steps with action numbers
